...
class ResidualEnsemble(nn.Module):
    """Ensemble de K MLP résiduels (article Sec IV-A-3 : K=3, seeds différents).

    La prédiction finale est la MOYENNE empirique des K membres — ce qui réduit la
    variance épistémique due au faible buffer de données (cf. article)."""
...
def _spectral_norm_penalty(model: nn.Module) -> torch.Tensor:
    """Σ_l ‖W^l‖₂ (norme spectrale = plus grande valeur singulière de chaque couche).

    C'est la régularisation de l'article (Sec III-D) : elle borne la constante de
    Lipschitz du réseau et améliore la généralisation hors-distribution. À NE PAS
    confondre avec le weight-decay (norme L2 des poids)."""
    pen = None
    for m in model.modules():
        if isinstance(m, nn.Linear):
            sn = torch.linalg.matrix_norm(m.weight, ord=2)
            pen = sn if pen is None else pen + sn
    return pen
...
def build_residual_dataset(log: dict, window_sec: float | None = None):
    ...
    # feature de poussée = c = T/m (mass-normalisée), comme u=[c, ω] de l'article (Sec III-D)
    c = (T_N / MASS)[:, None]
    X = np.concatenate([p, R.reshape(-1, 9), v, c, omega], axis=1)[mask]
    return X.astype(np.float32), y[mask].astype(np.float32)


def fit_residual(X, y, *, epochs=100, lr=1e-2, beta=1e-4, hidden=128,
                 n_models=3, device="cpu", seed0=0, verbose=True):
    """Entraîne un ENSEMBLE de `n_models` MLP résiduels (article : 3) et renvoie un
    `ResidualEnsemble` (moyenne des prédictions).

    Loss par membre = MSE + β·Σ‖W^l‖₂ (norme spectrale, Sec III-D). Chaque membre
    part d'un seed différent ; full-batch (le buffer est petit, ~100 échantillons).
    `epochs` = 100 itérations comme dans l'article (Sec IV-A-3)."""
    Xt = torch.from_numpy(X).to(device)
    yt = torch.from_numpy(y).to(device)
    models = []
    for k in range(n_models):
        torch.manual_seed(seed0 + k)                       # seeds différents (article)
        model = ResidualDynamicsMLP(hidden=hidden).to(device)
        opt = torch.optim.Adam(model.parameters(), lr=lr)  # pas de weight_decay : reg = spectrale
        mse = torch.tensor(0.0)
        for ep in range(epochs):
            pred = model(Xt)
            mse = (pred - yt).pow(2).mean()
            loss = mse + beta * _spectral_norm_penalty(model)
            opt.zero_grad(); loss.backward(); opt.step()
        if verbose:
            print(f"  [residual] membre {k}: mse={mse.item():.4f}")
        models.append(model)
    if verbose:
        print(f"  [residual] ensemble de {n_models} (moyenne)  sur {len(X)} échantillons")
    return ResidualEnsemble(models).to(device)
